  ...
  return new Promise((resolve) => {
    ...
    const req = https.request(opts, res => {
      ...
      res.on('end', () => {
        ...
        if (res.statusCode === 204) { resolve({ status: 204, data: null, headers: headersOut }); return; }
        try { resolve({ status: res.statusCode, data: JSON.parse(d), headers: headersOut }); }
        catch { resolve({ status: res.statusCode, data: d, headers: headersOut }); }
      });
    });
    req.on('error', (err) => {
